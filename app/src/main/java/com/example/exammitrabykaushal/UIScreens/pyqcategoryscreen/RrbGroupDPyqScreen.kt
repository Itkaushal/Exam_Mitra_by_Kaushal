...
fun GroupDPyqScreen(
    ...
    val papers = getDummyGroupdPapers(examName)
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Column {
                        Text(examName, fontWeight = FontWeight.SemiBold)
        ...
        Column(
            ...
            LazyColumn(
                ...
                items(papers) { groupdpaper ->
                    GroupDPaperItemCard(groupdpaper, onClick = {
                        loadPdfFromFirebaseGroupD(groupdpaper.key) { pdfUrl ->
                            if (pdfUrl != null) {
...
@RequiresApi(Build.VERSION_CODES.O)
fun loadPdfFromFirebaseGroupD(paperKey: String, callback: (String?) -> Unit) {

    FirebaseDatabase.getInstance()
        .getReference("RRB_GROUPD_PYQ")
        .child(paperKey)
        .get()
...
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun ViewPdfScreenGroupD(pdfUrl: String, onBack: () -> Unit) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("GROUP-D Previous Paper") },
                navigationIcon = {
                    IconButton(onClick = onBack) {
...
@Composable
fun GroupDPaperItemCard(paper: GroupDTestPaper, onClick: () -> Unit) {
    Card(
        modifier = Modifier
            .fillMaxWidth()
            .clickable { onClick() },
...
fun getDummyGroupdPapers(category: String): List<GroupDTestPaper> {
    return listOf(
        GroupDTestPaper("groupd_pyq_1", "RRB GROUP-D Shift 1", "10-Dec 2024", 100, "120 Min"),
        GroupDTestPaper("groupd_pyq_2", "RRB GROUP-D Shift 1", "12-Dec 2024", 100, "120 Min"),
        GroupDTestPaper("groupd_pyq_3", "RRB GROUP-D Shift 1", "14-Dec 2024", 100, "120 Min"),
        GroupDTestPaper("groupd_pyq_4", "RRB GROUP-D Shift 1", "16-Dec 2024", 100, "120 Min"),
        GroupDTestPaper("groupd_pyq_5", "RRB GROUP-D Shift 1", "18-Dec 2024", 100, "120 Min"),
